...
if __name__ == '__main__':
    ...
    num_neur = 150
    rate = 15
    data_folder = r'C:\Users\ron\OneDrive\Documents\Masters\Parnas\temporal-coding\Testing'
    # Loading test data

    with open(rf'{data_folder}\n{num_neur}_r{rate}.pickle', 'rb') as f:
        all_samples = pickle.load(f)
    false_labels = all_samples['labels'] == 0
    true_labels = all_samples['labels'] == 1
    
    spike_times1 = all_samples['data'][true_labels][0]
    spike_times2 = all_samples['data'][true_labels][1]


    efficacies = 1.8 * np.random.random(num_neur) - 0.50
