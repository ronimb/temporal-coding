...
class Tempotron:
    ...
    @convert
    def train_sample_py(self, sample, label, learning_rate=1e-4):
        self.update_counts(sample)
        counts = self.counts
        self.prep_network()
    ...
    def train_py(self, samples, labels, learning_rate=1e-4, n=[], iter=1):
        ...
        for _ in range(iter):
            for i in inds:
                print(i)
                self.train_sample_py(samples[i], labels[i], learning_rate)
...
def cycle(tempotron, samples, learning_rate=1e-4, n=[], iter=1):
    ...
    tempotron.train_py(samples['data'], samples['labels'], n=n, iter=iter)
